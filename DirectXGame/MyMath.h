...
Matrix4x4 MakeAffineMatrix(Vector3 scale, Vector3 rotate, Vector3 translate) {
	...
	Matrix4x4 rotateXYZMatrix = Multiply(Multiply(rotateZMatrix, rotateYMatrix), rotateXMatrix);

	Matrix4x4 translateMatrix = MakeTranslateMatrix(translate);

	// 各変換行列を合成してアフィン変換行列を作成
	affineMatrix = Multiply(Multiply(scaleMatrix, rotateXYZMatrix), translateMatrix);
...
Vector3 Reflect(const Vector3& input, const Vector3& normal) {
	// normalは正規化されていると仮定する
	float dotProduct = input.Dot(normal);
...
Vector3 Project(const Vector3& v1, const Vector3& v2) {
	...
	return v2 * (dotProduct / lengthSquared);
}